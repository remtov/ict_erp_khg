 ...
 * 
  <servlet>
    <servlet-name>TicketMovieServlet</servlet-name>
    <servlet-class>com.ict.erp.servlet.TicketMovieServlet</servlet-class>
  </servlet>
  <servlet-mapping>
    <servlet-name>TicketMovieServlet</servlet-name>
  ...
  url-pattern은 클라어인트의 요청에 대한 패턴으로 예를들어
     ...
     위의 url-pattern모두 TicketMovieServlet을 호출한다.
  localhost이후부터 /music/ <--이 들어가 있으면 만족함.
 */
public class TicketMovieServlet3 extends HttpServlet {
     ...
     * doGet 메소드는 클라이언트에서 get방식으로 보냈을때 실행되는 함수이다.
	  	예를들어 localhost/music/TicketMovie 라는 url로 method방식을 get으로 요청할경우
	  	web.xml에 기술되어있는 url-pattern /music/*을 만족하기 때문에
	  	TicketMovieServlet을 생성(생성되어있을경우엔 다시 생성안함) 하고
	  	doGet메소드를 자동으로 호출한다.
     */
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			log.error(tm);
			log.fatal(tm); 
			//클라이언트의 요청이 TicketMovie일경우(uri : localhost/music/TicketMovie)
